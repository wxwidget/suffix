...
public:
    ...
    CloneString(const CloneString& a):start(a.start),holdMem(false)
    {
        start = strdup(a.start);
    }
    CloneString(const char* b, bool alloc = false): start(b), holdMem(false)
    {
        if (alloc) start = strdup(b);
    }
    CloneString():start(NULL), holdMem(false)
    {
    }
    ~CloneString()
    {
    ...
    friend bool operator==(const CloneString& a, const CloneString& b)
    {
        return strcmp(a.start, b.start) == 0;
    }
    void unhold() { holdMem = false;};
    const char* start;
    bool holdMem;
};
...
public:
    ...
    inline unsigned hash(const CloneString& cstr)
    {
        register unsigned int h = 5381;
